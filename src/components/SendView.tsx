...
const SendView: Component<{ setMode: (mode: string) => void }> = (props) => {
  ...
  const initConnection = async () => {
    const code = await ObjSync.generateCode();
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate)
        ObjSync.emit("candidate", { code, candidate: e.candidate });
    };

    // Wait for receiver to join before sending offer
    ObjSync.emit("create-room", { code });

    ObjSync.on("receiver-joined", async () => {
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      ObjSync.emit("offer", { code, offer });
    });

    ObjSync.on("answer", (answer) =>
      pc.setRemoteDescription(new RTCSessionDescription(answer)),
    );
    ObjSync.on("candidate", (cand) =>
    ...
    <div class="w-full space-y-4">
      ...
      >
        ...
        <p class="text-sm font-medium text-zinc-300 lexend-400 text-center">
          {file() ? "Replace selected file" : "Click or drag file to upload"}
        </p>
        <p class="text-xs text-zinc-600">Supports any file type up to 5GB</p>
      </div>
      ...
      <div class="relative w-full overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl">
        ...
        <div class="p-6 overflow-x-auto">
          <pre class="font-mono text-sm leading-relaxed text-zinc-300 antialiased selection:bg-zinc-800 selection:text-white">
            <code>
              <span class="text-zinc-500 mr-2">1 |</span> Code := <br />
              <span class="text-zinc-500 mr-2">2 |</span> <span class="text-emerald-400">{generatedCode()}</span>
            </code>
          </pre>
        </div>
      </div>
